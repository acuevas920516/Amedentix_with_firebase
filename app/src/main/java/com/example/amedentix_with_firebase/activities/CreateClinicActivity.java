 ...
 */
public class CreateClinicActivity extends AppCompatActivity implements PermissionsListener, OnMapReadyCallback {
...
// Initialize the mapboxMap view
        ...
        latitude = 0;
        longitude = 0;
        save = (Button) findViewById(R.id.save_clinic);
        save.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {

                Map<String,Object> object = new HashMap<>();
                object.put("user_id",firebaseAuth.getCurrentUser().getUid());
                object.put("latitude",String.valueOf(latitude));
                object.put("longituda",String.valueOf(longitude));
                object.put("created_at", FieldValue.serverTimestamp());
                firebaseFirestore.collection("Clinics").add(object).addOnCompleteListener(new OnCompleteListener<DocumentReference>() {
                    @Override
                    public void onComplete(@NonNull Task<DocumentReference> task) {
                        if (task.isSuccessful()) {
                            startActivity(new Intent(CreateClinicActivity.this, ClinicData.class));
                            finish();
                        }
                    }
                });
    ...
    public void onMapReady(@NonNull final MapboxMap mapboxMap) {
        CreateClinicActivity.this.mapboxMap = mapboxMap;
...
// Transform the appearance of the button to become the cancel button
                            selectLocationButton.setBackgroundColor(
                                    ContextCompat.getColor(CreateClinicActivity.this, R.color.colorAccent));
...
// Switch the button appearance back to select a location.
                            selectLocationButton.setBackgroundColor(
                                    ContextCompat.getColor(CreateClinicActivity.this, R.color.colorPrimary));
...
// If the geocoder returns a result, we take the first in the list and show a Toast with the place name.
                            mapboxMap.getStyle(new Style.OnStyleLoaded() {
                                ...
                                public void onStyleLoaded(@NonNull Style style) {
                                    if (style.getLayer(DROPPED_MARKER_LAYER_ID) != null) {
                                        Toast.makeText(CreateClinicActivity.this,
